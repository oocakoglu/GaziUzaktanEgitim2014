...
var LCSKChat = function () {
    ...
    function setDefaults() {
        ...
        options.offlineTitle = 'İletişime Geç!';
        options.onlineTitle = 'Online Destek!';

        options.waitingForOperator = 'Teşekkürler, Operatörümüz en kısa manada size cevap verecektir';
        options.emailSent = 'Emailiniz gönderilmiştir. Mümkün Olan en kısa zamanda size döneceğiz';
        options.emailFailed = 'Emailniz gönderilemedi. <br /><br />Lütfen konuyla alakalı arıza kaydı oluşturun.';
        ...
        if (state) {
            ...
            if (!requestChat) {
                $('#chat-box').html(
                    ...
                    '<p>Bir sorununuz mu var? Bize Sorabilirsiniz!</p><p>Sorununuzu yazın ve ENTER butonuna basın.</p></div>' +
    ...
    myHub.client.setChat = function (id, agentName, existing) {
        ...
        if (existing) {
            ...
            $('#chat-box-msg').append('<p><strong>' + agentName + '</strong>  operötörü ile görüşmeniz devam ediyor</p>');
        } else {
            $('#chat-box-msg').append('<p>Şu an <strong>' + agentName + '</strong> operötörü ile görüşüyorsunuz</p>');
        }
    };
